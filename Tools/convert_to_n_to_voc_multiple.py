...
def generate_xml(preds, img_file_name, shape, output_dir, output_label, output_path_xml):
    ...
    folder.text = output_label.split("/")[-1].split("_")[0]
    ...
    for i in range(len(bboxes)):
    	...
    	name.text = output_label.split("/")[-1]
...
if __name__ == "__main__":
    nat_json_path = Path("nat_json")
    for json_folder in nat_json_path.iterdir():
        input_dir = str(json_folder)
        output_dir = "voc_xml/"+ input_dir.split("/")[-1].split("_")[0]
